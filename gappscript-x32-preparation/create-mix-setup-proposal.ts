 ...
 **/
const createMixSetupProposal = (serviceType, planID, channelsOfTheMixer, positionSettings, user, pwd) => ({
    const url = URL_PEOPLE_POSITIONS.replace(SERVICE_TYPE, serviceType).replace(LINES, 50).replace(PLAN_ID, planId);
    return peoplePositionsToTable(loadPeopleAndPositions(url, user, pwd));
});
